...
def finetune(model_name, model_weights, filename, dataset_dir, epoch, batch_size, img_shape=(256, 256), checkpoints_dir="checkpoints"):
    ...
    model.fit(epoch, train_info, val_info, save_model_dir=checkpoints_dir)
...
def main():
    ...
    if model_type == 0:
        ...
        train("MobileNetV2", batch_size, epoch, train_dir, val_dir, img_shape, chepoints_dir)
    elif model_type == 1:
        filename = ""
        model_weights = ""
        dataset_dir = ""
        finetune(model_weights, filename, dataset_dir, epoch, batch_size, img_shape, chepoints_dir)
    else:
